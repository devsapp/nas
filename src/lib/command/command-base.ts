...
export default class CommandBase {
  ...
  fileChunkUploadPath(serviceName: string): string {
    return `/proxy/${getServiceName(serviceName)}/${NAS_OPERATION_HELPER_FUNCTION_NAME}/file/chunk/upload`;
  }

  commandPath(serviceName: string): string {
    return `/proxy/${getServiceName(serviceName)}/${NAS_OPERATION_HELPER_FUNCTION_NAME}/commands`;
  }

  async callCommands(serviceName: string, cmd: string): Promise<any> {
    const p = this.commandPath(serviceName);
