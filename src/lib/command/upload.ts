...
import { checkFileExist, resolveLocalPath } from '../utils';
...
export default class Upload extends CommandBase {
  async cpFromLocalToNas(props: ICommandProps, apts: IApts) {
    ...
    const localFileStat = await checkFileExist(localResolvedSrc);
    if (!localFileStat) throw new Error(`${localResolvedSrc} is not a file or directory`);
  ...
  async uploadFolder(
    ...
    const excludes = [path.relative(process.cwd(), path.join(process.cwd(), '.s'))];
    await zip({
  ...
  private async uploadFile(
    ...
    // 创建文件
    const vm = spinner(`Start create file: ${actualDstPath}`);
    ...
    return new Promise((resolve) => {
      const unzipQueue = async.queue(async (unzipFiles, next) => {
        try {
          ...
          const res = await super.callCommands(serviceName, cmd);
    ...
    return new Promise((resolve) => {
      ...
      const uploadQueue = async.queue(async (offSet, callback) => {
        try {
          ...
          const urlPath = super.fileChunkUploadPath(serviceName);
  ...
  private async checkUploadDstPath(
    ...
    // 如果本地是文件夹，远端是文件
    if (dstPathIsFile && isDirectory) throw new Error(localDirButDstFileError);

    // 远端地址已经存在
    if (dstPathExists) {
      if (isDirectory) {
        // 如果本地是文件夹并且不以 / 结尾，上传`当前目录下`所有的文件到远端地址
        if (localPathEndWithSlash) return dstPath;

        // 重新 check newDstPath 是否在远端存在
        const { data: stats } = await super.callStats(serviceName, newDstPath);
        if (!stats.exists || stats.isDir) return newDstPath;
        throw new Error(localDirButDstFileError);
      }
      // 如果本地不是文件夹，check 新的目标地址
      if (dstPathIsDir) {
        const { data: stats } = await super.callStats(serviceName, newDstPath);
        if (stats.isFile && noClobber) throw new Error(`${newDstPath} already exists`);
        if (!stats.exists || stats.isFile) return newDstPath;
    ...
    // 远端地址不存在
    if (!parentDirOfDstPathExists) throw new Error(dstParentPathNotExistAndDstNotExistError); // 父目录不存在
    if (isDirectory) return dstPath; // 需要上传的是文件夹
    if (dstPathEndWithSlash) throw new Error(localFileAndDstNotExistError); // 需要上传的是文件，远端以斜线结尾
