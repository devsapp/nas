...
import { promptForConfirmContinue, sleep } from './utils';
...
export default class Nas {
  static checkDeployProps(props: IDeployProps) {
    ...
    if (_.isEmpty(props?.mountPoints) && _.isEmpty(zoneId)) throw new Error('Parameter is missing zoneId');
  }
  ...
  async findNasFileSystem(nasName: string, zoneId?: string, vpcConfig?: IVpcConfig): Promise<FileSystemKey> {
    ...
    if (!_.isEmpty(fileSystems)) {
      const findRs = this.findFileSystemUseVpcConfig(fileSystems, vpcConfig);
      logger.debug(`findFileSystemUseVpcConfig res: ${JSON.stringify(findRs)}`);
      return findRs;
    }
  }
  ...
  private findFileSystemUseVpcConfig(fileSystems: any[], vpcConfig: IVpcConfig): FileSystemKey {
